...
# determines how values in the repl are printed
# options are 'true','false','false-to-str'
global print_as_lisp
print_as_lisp = 'true'

def set_repl_print_mode(modestr):
    if modestr in ['true','false','false-to-str']:
        print_as_lisp = modestr
        return True
    else:
        return False
...
class Procedure(object):
    ...
    def __call__(self, *args):
        return _eval(self.exp, Env(self.parms, args, self.env))
...
def to_string(x):
    if print_as_lisp == 'true':
        ...
        if x is True:
            ...
            return str(x)
    elif print_as_lisp == 'false-to-str':
        return str(x)
    elif print_as_lisp == 'false':
        return x
    else:
...
def add_globals(self):
    ...
    self.update({
        ...
        "eval": lambda x: _eval(expand(x)), "load": lambda fn: load(fn), "call/cc": callcc,
